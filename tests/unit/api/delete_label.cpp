...
int main(int argc, char ** argv)
{
  ...
  perf_taint::register_variable(&x3, VARIABLE_NAME(x3));
  ...
  perf_taint::delete_label(&x4, "x1");
  assert(!perf_taint::has_label(&x4, "x1"));
  assert(!perf_taint::has_label(&x4, "x2"));
  assert(!perf_taint::has_label(&x4, "x3"));

  // delete non-existing label
  int x5 = x1;
  assert(perf_taint::has_label(&x5, "x1"));
  perf_taint::delete_label(&x5, "x2");
  assert(perf_taint::has_label(&x5, "x1"));
  assert(!perf_taint::has_label(&x5, "x2"));
  assert(!perf_taint::has_label(&x5, "x3"));

  // delete one of two labels
  int x6 = x1_x2;
  ...
  assert(perf_taint::has_label(&x6, "x2"));
  assert(!perf_taint::has_label(&x6, "x1"));
  assert(!perf_taint::has_label(&x6, "x3"));

  // delete one of two labels, second one
  int x7 = x2_x3;
  assert(perf_taint::has_label(&x7, "x2"));
  assert(perf_taint::has_label(&x7, "x3"));
  perf_taint::delete_label(&x7, "x3");
  assert(perf_taint::has_label(&x7, "x2"));
  assert(!perf_taint::has_label(&x7, "x1"));
  assert(!perf_taint::has_label(&x7, "x3"));

  // delete two of three labels
  int x8 = x1_x2_x3;
  assert(perf_taint::has_label(&x8, "x1"));
  assert(perf_taint::has_label(&x8, "x2"));
  assert(perf_taint::has_label(&x8, "x3"));
  perf_taint::delete_label(&x8, "x3");
  perf_taint::delete_label(&x8, "x1");
  assert(perf_taint::has_label(&x8, "x2"));
  assert(!perf_taint::has_label(&x8, "x1"));
  assert(!perf_taint::has_label(&x8, "x3"));

  // delete two of three labels, differen order
  x8 = x1_x2_x3;
  assert(perf_taint::has_label(&x8, "x1"));
  assert(perf_taint::has_label(&x8, "x2"));
  assert(perf_taint::has_label(&x8, "x3"));
