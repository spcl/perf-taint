...
namespace perf_taint {
    ...
    llvm::StringRef DfsanInstr::getPassName() const
    {
      return "PerfTaintInstrument";
    }
            ...
            llvm::CallGraphNode * cg_node, int override_counter)
    {
        ...
        for(auto & callsite : *cg_node)
        {
            ...
            //llvm::errs() << "Function call: " << *call << " inside " << f.getName() << '\n';
            //if(called_f) {
            //  llvm::errs() << "Called function: type? " << *called_f->getFunctionType() << '\n';
            //  llvm::errs() << "Called function: vararg? " << called_f->isVarArg() << '\n';
            //  llvm::errs() << "Called function: analyzable? " << is_analyzable(*m, *called_f) << '\n';
            //  llvm::errs() << "Called function: name? " << called_f->getName() << '\n';
            //}
            //else
            //  llvm::errs() << "Called function: not available" << '\n';
        ...
        if(EnableSCEV) {
          llvm::Function* analyzed_f = m->getFunction(function_name);
  ...
  llvm::Pass* createPerfTaintInstrumentationPass()
  {
    return new DfsanInstr();
  } 
}
